...
codebase has a lot of those on purpose, and copying them here would produce two records that
...
**Held by:** `flock` for migration and `_raw`, and `Curator`'s prefix ownership for the context
...
harness reports out loud when the correction loop is not connected end to end, rather than
...
(punctuation, bare booleans, prefix terms, CJK detection), and CI for `eval/run.sh`.
...
suite. A rule that cannot be checked that way should name its enforcer, so that nobody mistakes a sentence for a
...
- **Which root files the sidebar pins.** `contextFiles` listed four; `buildTree`'s "already
  ...
  the top and once down among the loose root files, and both rows carried the same `.tag`, so
...
been sitting in the body of four files in the Files tab. Those are the markers that function
exists to hide, and nobody had read it as a symptom.
...
maintain. Where the two callers need different answers, say so in the type rather than in
...
recorded, so leave it there. That comment habit is described in
