...
**実装状態**: 未着手。この節は決定であって、完了報告ではありません。

**やること**（順序に意味があります。壊れた状態のまま削除の判断を下さないため）:

1. 日記が3日止まっている原因を特定する
2. `writeDailyFile` の no-op を解消し、`daily/` を `full.md` のコピーから日記本体に戻す
3. `CurrentState.recentActions` の clipboard 混入を直す（file と MCP の両方に効く）
4. Files タブを畳む
