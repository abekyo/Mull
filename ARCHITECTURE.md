...
| 9 | Who owns a vault file is decided in one place (`VaultOwnership`), and every writer asks it. (It was named for two askers, the Files tab and the MCP server; the Files tab went on 2026-08-15 — DIRECTION §6.2 — and the rule is unchanged, which is the point of it not living in a view.) | [`VaultOwnershipTests`](Tests/VaultOwnershipTests.swift), in particular `testCuratedDirectoriesAreTheOnesWithACuratorBehindThem` and `testAnAgentMayNotStampOverACuratedFile` |
