...
| 9 | Who owns a vault file is decided in one place (`VaultOwnership`), and the Files tab and the MCP server both ask it. | [`VaultOwnershipTests`](Tests/VaultOwnershipTests.swift), in particular `testCuratedDirectoriesAreTheOnesWithACuratorBehindThem` and `testAnAgentMayNotStampOverACuratedFile` |
