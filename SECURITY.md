...
| Moving the vault | `~/mull` can be relocated with `defaults write com.mull.app vaultPath <absolute path>`. There is no UI for it, and it exists for one job: putting the vault inside a folder something else already syncs, which is your job rather than mull's. Taking it changes two of the promises above, so read this before you do. The **database does not move** — it stays at `~/Library/Application Support/mull/`, because SQLite in a folder a sync service uploads on its own schedule is the documented way to corrupt one (a copy taken mid-transaction holds some old and some new pages, and the `-wal` file separated from its database loses committed transactions; see [sqlite.org/howtocorrupt](https://www.sqlite.org/howtocorrupt.html) §1.2 and §1.4). And the `0700` root above stops protecting anything: mull still sets that mode on wherever the vault now is, but the folder it sits in belongs to the sync service, and the vault's contents — window titles, page text, what you typed, condensed — go wherever that service sends them. If you point this at iCloud Drive, they are on Apple's servers. mull does not sync, and this key does not make it sync: nothing in it watches the vault for changes arriving from elsewhere, and the 60s pass rewrites `me.md`, `now.md`, `full.md` and `mull.md` from *this* machine's database. One Mac recording, others reading, is the shape this supports. Two Macs recording into one synced folder will overwrite each other's copy of those four files every minute. |
| API keys | macOS Keychain, with `kSecAttrAccessibleWhenUnlockedThisDeviceOnly` — deliberately not an iCloud Keychain item, so a key stays on the Mac you typed it into and a second machine asks again. Never written in plaintext. |
