...
- **App Name**: Dream
- **Subtitle**: The diary that writes itself.
...
**自分で書かなくていい日記。しかもAIが毎晩読んで翌日の仕事に活かしてくれる。**

日記は人類最古の「第二の脳」。でも誰も続かない。書くのが面倒で、何を書けばいいかわからなくて、書いても読み返さなくて、読み返しても活用できない。

Dreamは4つ全部を解決する:

1. **書くのが面倒** → 書かなくていい。PCを使うだけで自動で記録される
2. **何を書けばいいか** → 全部記録してAIが構造化する
3. **読み返さない** → AIが勝手に読んで活用してくれる
4. **活用できない** → MCPでAIに直接渡る。行動に変わる

### How Dream Changes Your Life

```
Before Dream:
  月曜の朝: 「先週何やったっけ...」→ Slackとメールを30分漁る
  AI使用時: 「私はSwift開発者で、今このプロジェクトの...」→ 毎回5分の説明
  評価面談: 「今四半期の成果は...」→ 思い出せない。過小評価される
  確定申告: 「経費の記録が...」→ レシートとメールを3日かけて掘る
  転職活動: 「職務経歴書に何書こう...」→ 2年分の仕事を思い出せない

After Dream:
  月曜の朝: パネルを開く → 先週の全活動が構造化されている → 30秒で把握
  AI使用時: 何も言わない → AIが既に知っている → 即座に的確な回答
  評価面談: ~/Dream/daily/ → 3ヶ月分の全成果が日次で残っている → 正当に評価される
  確定申告: 検索1回 → 全ての仕事・取引先・やり取りが出る → 1時間で完了
  転職活動: me.md + 6ヶ月のTimeline → 職務経歴書の素材が全部ある
```
...
4つで十分。5つ目のタブが必要になったら、既存の4つのどこかが肥大化している証拠。
