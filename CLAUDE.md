...
**mullはデータ層であり、AIの頭脳ではない。**

- AIに「考えさせる」のはClaude/ChatGPT/Cursorの仕事
- mullの仕事は「AIに渡す記憶を整理する」こと
- mullが賢くなるのではなく、mullを読んだAIが賢くなる

> **スコープ付きチャットについて（v2）**: mull内のチャットは「汎用チャットボット」ではない。あなたの記録（me/now/projects）に根拠を置いて「自分について」答え、記録の再整理を指示する窓口。雑談や一般質問が来たら断ってClaude/ChatGPTに誘導する。これはデータ層の原則を破らない——生データを再処理する操作面であって、AIの頭脳ではない。重い思考はMCP経由でmullを読むClaude/ChatGPTがやる。
